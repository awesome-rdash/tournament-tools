...
function encode_name($name) {
	...
	echo "<strong>Clé API utilisée : $api_key </strong>";
	echo "<br />";
	echo "<br />";
	...
	//On va recuperer les IDs de tous les joueurs. 
	//Limite API : 40 joueurs par call. - 8 calls necessaires pour 320 joueurs
		...
		foreach($playersToCheck_array as $player) {
			if (array_key_exists($player, $result)) {
				...
				$playersID[$id] = array (
					...
					"id" => $result[$player]['id'],
					"summonerLevel" =>$playersName[$player]['summonerLevel'],
					"ranked_solo" => "",
					"note" => "");
			} else {
				echo "Le joueur <strong>$player</strong> est introuvable dans la BDD Riot<br />";
			}
		}
...
foreach ($players as $player) {
	...
	echo "<td style=\"text-decoration:underline; background-color: ". get_background_color($player['note']) .";\" ><mark>" . $player['note'] . "</mark></td>";
